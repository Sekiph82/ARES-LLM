...
DEFAULT_DESIGN_SYSTEM = """# Ares Design Contract
...
def design_path(repo: Path) -> Path:
    return repo / "DESIGN.md"


def ensure_design_system(repo: Path) -> Path:
    path = design_path(repo)
    if not path.exists():
        path.write_text(DEFAULT_DESIGN_SYSTEM, encoding="utf-8")
    return path


def load_design_system(repo: Path) -> str:
    path = ensure_design_system(repo)
